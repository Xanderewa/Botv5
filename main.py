...
def build_context_hash(candles: List["Candle"], decision: "StrategyDecision") -> str:
    last = candles[-1]
...
class Config:
    ...
    run_self_test: bool = os.getenv("RUN_SELF_TEST", "1") == "1"
...
class TelegramNotifier:
    ...
    def send(self, text: str) -> bool:
        ...
        r = self.session.post(url, data=payload, timeout=(10, 15))
        r.raise_for_status()
        return True
...
class BotLogger:
    def __init__(self, level=logging.INFO):
        self.logger = logging.getLogger("pro_signal_bot")
    ...
    def evaluate(self, candles: List[Candle], mode: str, cfg: Config) -> StrategyDecision:
        ...
        if not fast or not slow or current_rsi is None or current_atr is None or avg_volume is None:
            return StrategyDecision(
                ...
                confidence=0.0,
                validations_failed=["insufficient_data"],
            )
...
class SignalManager:
    ...
    def evaluate(self, decision: StrategyDecision, candles: List[Candle]) -> ManagerDecision:
        ...
        ctx = build_context_hash(candles, decision)
...
class ProSignalApp:
    ...
    def run_forever(self):
        interval = max(15, seconds_for_timeframe(self.cfg.timeframe) // 6)
        while True:
            self.run_once()
            time.sleep(interval)
